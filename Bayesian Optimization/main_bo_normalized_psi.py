...
def objective_np(shape, A=DEFAULT_A, n_quad=2048):
    """Evaluate normalized_psi_pressure on a plain NumPy-like shape vector.
...
def negative_normalized_psi_pressure_np(shape, A=DEFAULT_A, n_quad=2048):
...
def optimize_shape_bayesian(
    ...
    objective = lambda x: negative_normalized_psi_pressure_np(
    ...
    best_normalized_psi_pressure = objective_np(best_shape, A=A, n_quad=n_quad)
    ...
    if shape_iters:
        ...
        first_normalized_psi_pressure = objective_np(first_shape, A=A, n_quad=n_quad)
    else:
        first_shape = best_shape
        first_normalized_psi_pressure = best_normalized_psi_pressure

    return {
        "initial_shape": jnp.asarray(first_shape),
        "initial_normalized_psi_pressure": first_normalized_psi_pressure,
        "raw_initial": unconstrained_from_bounded(jnp.asarray(first_shape), bounds),
        "shape": jnp.asarray(best_shape),
        "normalized_psi_pressure": best_normalized_psi_pressure,
...
def run_unit_tests(
    ...
    optimum_value = float(result["normalized_psi_pressure"])
    ...
    print(f"  Max perturbed normalized_psi_pressure: {max_perturbed_value:.12g}")
    print(f"  Optimum normalized_psi_pressure: {optimum_value:.12g}")
    ...
    return {
        "max_perturbed_normalized_psi_pressure": max_perturbed_value,
        "checks": checks,
    }
...
def print_final_gradient_diagnostics(
    ...
    print(f"  Objective: normalized_psi_pressure = {diagnostics['value']:.12g}")
...
def main(argv=None):
    ...
    print(f"Initial normalized_psi_pressure: {float(result['initial_normalized_psi_pressure']):.12g}")
    print(f"Final shape: {_format_shape(result['shape'])}")
    print(f"Final normalized_psi_pressure: {float(result['normalized_psi_pressure']):.12g}")
